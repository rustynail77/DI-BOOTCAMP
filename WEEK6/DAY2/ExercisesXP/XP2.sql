...
-- SELECT first_name, address, phone FROM customer INNER JOIN address ON customer.address_id = address.address_id WHERE address.district ILIKE 'Texas';
...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title ILIKE 'fight club';
...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title ILIKE 'fi%';
...
-- SELECT title, rental_rate FROM film ORDER BY rental_rate ASC OFFSET 10;???
...
-- SELECT payment_id, amount, payment_date, first_name FROM payment INNER JOIN customer ON payment.customer_id=customer.customer_id ORDER BY customer.customer_id;

-- 13. You need to check your inventory. Write a query to get all the movies which are not in inventory.
-- SELECT film.film_id, title, inventory.film_id FROM film LEFT JOIN inventory ON film.film_id=inventory.film_id WHERE inventory.film_id IS NULL;
